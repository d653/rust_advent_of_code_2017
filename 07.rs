...
";
    ...
    let mut w = HashMap::<String,i32>::new();
    let mut top = HashMap::<String,String>::new();
    let mut toc = HashMap::<String,Vec<String>>::new();
    
    let re1 = Regex::new(r"^([a-z]*) \(([0-9]*)\)").unwrap();
    ...
    let mut cur = top.iter().next().unwrap().1.clone();
    while let Some(s) = top.get(&cur) {
        cur = s.clone();
    }
    println!("{}",cur);
...
fn weight(w : &HashMap<String,i32>, toc : &HashMap<String,Vec<String>>, cur : String ) -> i32{
    let ws : Vec<_> = toc[&cur].iter().map(|s|{
        (s,weight(w,toc,s.clone()))
    }).collect();
    
    if ws.iter().map(|&(_,b)|b).collect::<HashSet<_>>().len() > 1 {
        let a = ws.iter().max_by_key(|x|x.1).unwrap();
        let b = ws.iter().min_by_key(|x|x.1).unwrap();
        println!("{}",w[a.0]-a.1+b.1);
        std::process::exit(0);
    }

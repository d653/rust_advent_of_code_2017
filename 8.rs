...
";
    ...
    for s in x.lines() {
        ...
        let r1 = v[0].to_string();
        let id = v[1];
        let n1 :i32 = v[2].parse().unwrap();
        let r2 = v[4].to_string();
        let cmp = v[5];
        let n2 :i32 = v[6].parse().unwrap();
        
        let vr1 = *regs.entry(r1.clone()).or_insert(0);
        let vr2 = *regs.entry(r2.clone()).or_insert(0);
        if match cmp {
            ">" => {vr2 > n2},
            "<" => {vr2 < n2},
            "==" => {vr2 == n2},
    ...
    println!("{} {}",regs.values().max().unwrap(),max);
